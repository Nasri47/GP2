...
public final class UserRegisterUtils {

    private static final String LOG_TAG = FieldsListUtils.class.getSimpleName();

    private UserRegisterUtils() {
    }

    private static UserInfo extractFeatureFromJson(String fieldsListJSON) {
        if (TextUtils.isEmpty(fieldsListJSON)) {
            return null;
        }

        UserInfo fields = new UserInfo();
    ...
    public static UserInfo fetchfieldsData(String requestUrl) {
        // Create URL object
        ...
        UserInfo fieldItems = extractFeatureFromJson(jsonResponse);
